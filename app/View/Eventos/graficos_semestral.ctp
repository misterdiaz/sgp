...
echo $form->create($model = 'Evento', $options = array('action'=>'graficos_semestral'));
echo "<br/>";
...
echo $form->input("semestre", array('label'=>array("text"=>'Semestre:',"class"=>'span-2 derecha'), 'options'=>array('1'=>'Primer Semestre', '2'=>'Segundo Semestre'), 'empty'=>FALSE, 'class'=>'span-6 last', 'div'=>array('class'=>'input select span-8') ) );
echo $form->end(array('div'=>array('class'=>'submit span-4'), 'label'=>'Ver Estadisticas'));
if(!empty($this->data)){
	echo "<div class=''>";
	echo $html->link(
		$title= "Generar PDF", 
		$url = array('controller'=>'Eventos', 'action'=>'generarPdfSemestral', $this->data['Evento']['year'], $this->data['Evento']['semestre']), 
...
//$DataSet->AddSerie();
if($this->data['Evento']['semestre'] == 1){
	$DataSet->AddPoint(array("Ene", "Feb", "Mar", "Abr", "May", "Jun"), 'SerieM');
}else{
	$DataSet->AddPoint(array("Jul", "Ago", "Sep", "Oct", "Nov", "Dic"), 'SerieM');
}
...
$Test->setFontProperties("/Fonts/tahoma.ttf",10);  
$Test->Render("graficos/graficos_semestre_".$this->data['Evento']['semestre']."_$year.png");  
echo $html->image("../graficos/graficos_semestre_".$this->data['Evento']['semestre']."_$year.png");
?>
<br/><br/>
<? 
$DataSet = new pData;
...
$Test->Render("graficos/graficos_torta_tipo_eventos_semestre_".$this->data['Evento']['semestre']."_".$year.".png");
?>
<? 
$DataSet = new pData;
...
$Test->Render("graficos/graficos_torta_nacionalidad_semestre_".$this->data['Evento']['semestre']."_".$year.".png");
?>
<span class="span-11">
<h3>Gráfico de eventos según su tipo:</h3>
<? echo $html->image("../graficos/graficos_torta_tipo_eventos_semestre_".$this->data['Evento']['semestre']."_".$year.".png"); ?>
</span>
<span class="span-11 last">
<h3>Gráfico de eventos según su nivel:</h3>
<? echo $html->image("../graficos/graficos_torta_nacionalidad_semestre_".$this->data['Evento']['semestre']."_".$year.".png"); ?>
</span>
<? 
}
?>